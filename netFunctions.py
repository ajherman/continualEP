...
def train(net, train_loader, epoch, learning_rule):
    ...
    for batch_idx, (data, targets) in enumerate(train_loader):
        ...
        state.moveToDevice(net.device)

        if learning_rule == 'stdp':
            with torch.no_grad():
                if batch_idx%500==0:
                    s,deltas1, mps1 = net.forward(data,state,record=True)
                else:
                ...
                if batch_idx%500==0:
                    s, deltas2, mps2 = net.forward(data, state,target=targets, beta=beta,record=True)
                else:
                    s = net.forward(data,state,target=targets,beta=beta)
