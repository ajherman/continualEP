...
class VFcont(nn.Module):
    ...
    def forward(self, data, s, seq = None,  beta = 0, target = None, **kwargs):
        T = self.T
        Kmax = self.Kmax
        if len(kwargs) > 0:
            K = kwargs['K']
...
class EPdisc(nn.Module):
    ...
    def computeGradients(self, data, s, seq, beta):
        ...
        for i in range(self.ns - 1):
            if self.update_rule == 'cep-alt':
                # gradw.append((1/(beta*batch_size))*( torch.mm(torch.transpose(s[i] - seq[i], 0, 1), seq[i + 1]) +  torch.mm(torch.transpose(s[i],0,1),s[i+1]-seq[i+1]) ))
                gradw.append((1/(beta*batch_size))*( torch.mm(torch.transpose(s[i] - seq[i], 0, 1), s[i + 1]) +  torch.mm(torch.transpose(s[i],0,1),s[i+1]-seq[i+1]) )) # This does not work at all if we use seq instead of s for the activity terms
            elif self.update_rule == 'cep': # Original version
