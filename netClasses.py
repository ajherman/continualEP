...
#######################################################################
        ...
        if self.update_rule == 'stdp':
            ...
            gradw.append( (1/(beta*batch_size)) * ( torch.mm(torch.transpose(spike[-2],0,1),rho(s[-1])) - torch.mm(torch.transpose(rho(s[-2]),0,1), spike[-1]) ))
        ...
        elif self.update-rule == 'skewsym': #New
